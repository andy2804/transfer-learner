...
class TLStatistician:
    def __init__(self,
        ...
        self._keep_score_label_map = {'None': 'All', 'False': 'Rejected', 'True': 'Approved'}

    def append_obj_stats(self, label, ymin, xmin, h, w, tl_score, tl_keep):
        """
        :param label:
        ...
        self._objstats.append(ObjStats(
                label=label, ymin=ymin, xmin=xmin, h=h, w=w, tl_score=tl_score, tl_keep=tl_keep))

    def get_obj(self, label_filt=None, tl_keep_filt=None):
        return [obj for obj in self._objstats if
                (tl_keep_filt is None or tl_keep_filt == obj.tl_keep) and
    ...
    def num_obj(self, label_filt=None, tl_keep_filt=None):
        ...
        for obj in self._objstats:
            if (tl_keep_filt is None or tl_keep_filt == obj.tl_keep) and (
                    label_filt is None or label_filt == obj.label):
                count += 1
        return count

    def publish(self):
        # todo on google sheets or somewhere else
        pass
    ...
    def make_plots(self):
        ...
        self._full_dist_of_aspect_ratio()
        self._full_dist_of_tlscore()
        return

    def _full_dist_of_aspect_ratio(self, ):
        ...
        fig.savefig('aspectratio.pdf')
        plt.show()
        return

    def _full_dist_of_tlscore(self, ):
        n_categories = len(self._labels)
