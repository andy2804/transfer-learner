"""
author: Copyright 2016 The TensorFlow Authors. All Rights Reserved.
review: az
#todo check migration from slim to core tensorflow
...
Clone = namedtuple('Clone', ['outputs',  # Whatever model_fn() returned.
                             'scope',  # The scope used to create it.
                             'device',  # The device used to create.
                             ])

# Namedtuple used to represent a DeployedModel, returned by deploy().
DeployedModel = namedtuple('DeployedModel',
...
# todo move to magic_constants
# Default parameters for DeploymentConfig
_deployment_params_default = {'num_clones':         1,
                              'clone_on_cpu':       False,
...
def create_clones(config, model_fn, args=None, kwargs=None):
    ...
    deploying one or several clones on different GPUs and one or several
    replicas
    of such clones.
    The argument `model_fn` is called `config.num_clones` times to create the
    model clones as `model_fn(*args, **kwargs)`.
    If `config` specifies deployment on multiple replicas then the default
    tensorflow device is set appropriately for each call to `model_fn` and
    for the
    slim variable creation functions: model and global variables will be created
    on the `ps` device, the clone operations will be on the `worker` device.

    :param config: A DeploymentConfig object.
    :param model_fn: A callable. Called as `model_fn(*args, **kwargs)`
    :param args: Optional list of arguments to pass to `model_fn`.
    :param kwargs: Optional list of keyword arguments to pass to `model_fn`.
    :returns: A list of namedtuples `Clone`.
    """
    clones = []
    args = args or []
    kwargs = kwargs or {}
...
def _gather_clone_loss(clone, num_clones, regularization_losses):
    ...
    :param clone: A Clone namedtuple.
    :param num_clones: The number of clones being deployed.
    :param regularization_losses: Possibly empty list of regularization_losses
            to add to the clone losses.
    :returns A tensor for the total loss for the clone.  Can be None.
    """
    # The return value.
    sum_loss = None
...
def _optimize_clone(optimizer, clone, num_clones,
    ...
    Returns:
        A tuple (clone_loss, clone_grads_and_vars).
            - clone_loss: A tensor for the total loss for the clone.  Can be
            None.
...
def optimize_clones(clones, optimizer,
    ...
    :param clones: List of `Clones` created by `create_clones()`.
    :param optimizer: An `Optimizer` object.
    :param regularization_losses: Optional list of regularization losses. If
    None it
         will gather them from tf.GraphKeys.REGULARIZATION_LOSSES. Pass `[]` to
         exclude them.
    :returns : A tuple (total_loss, grads_and_vars).
    :return total_loss: A Tensor containing the average of the clone losses
            including the regularization loss.
    :return grads_and_vars: A List of tuples (gradient, variable) containing the
    ...
    for clone in clones:
        with tf.name_scope(clone.scope):
            clone_loss, clone_grad = _optimize_clone(
                    optimizer, clone, num_clones, regularization_losses,
                    **kwargs)
            if clone_loss is not None:
...
def deploy(config,
    ...
    """ Deploys a model across multiple clones.
    The deployment options are specified by the config object and support
    deploying one or several clones on different GPUs and one or several
    replicas
    of such clones.
    ...
    :param args: Optional list of arguments to pass to `model_fn`.
    :param kwargs: Optional list of keyword arguments to pass to `model_fn`.
    :param optimizer: Optional `Optimizer` object.  If passed the model is
    deployed
          for training with that optimizer.
    :param summarize_gradients: Whether or not add summaries to the gradients.
    :returns A `DeployedModel` namedtuple.
    """
...
class DeploymentConfig:
    ...
    def __init__(self,
                 ...
                 worker_job_name='worker',
                 ps_job_name='ps'):
        """ The config describes how to deploy a model across multiple clones
        and replicas. The model will be replicated `num_clones` times in each
        replica.
        If `clone_on_cpu` is True, each clone will placed on CPU.
        If `fake_multiple_gpus` is True, the model will only be replicated
        once on
        a single GPU. This trick enables larger batch sizes, necessary for
        training
        deep networks such as InceptionV3/V4, on a single GPU.
        If `num_replicas` is 1, the model is deployed via a single process.
        In that
        case `worker_device`, `num_ps_tasks`, and `ps_device` are ignored.
        If `num_replicas` is greater than 1, then `worker_device` and
        `ps_device`
        must specify TensorFlow devices for the `worker` and `ps` jobs and
        `num_ps_tasks` must be positive.

        :param num_clones: Number of model clones to deploy in each replica.
        :param clone_on_cpu: If True clones would be placed on CPU.
        :param replica_id: Integer.  Index of the replica for which the model is
              deployed.  Usually 0 for the chief replica.
        :param num_replicas: Number of replicas to use.
        :param num_ps_tasks: Number of tasks for the `ps` job. 0 to not use replicas.
        :param worker_job_name: A name for the worker job.
        :param ps_job_name: A name for the parameter server job.
        :raises ValueError: If the arguments are invalid.
        """
        if num_replicas > 1 > num_ps_tasks:
            raise ValueError(
        ...
        self._worker_device = '/job:' + worker_job_name if num_ps_tasks > 0 \
            else ''
    ...
    def clone_device(self, clone_index):
        ...
        Raises:
            ValueError: if `clone_index` is greater or equal to the number of
            clones".
        """
    ...
    def inputs_device(self):
        ...
        :returns A value suitable for `tf.device()`.
        """
        device = ''
        if self._num_ps_tasks > 0:
            device += self._worker_device
